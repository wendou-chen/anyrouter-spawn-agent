...
- Direct MCP `tools/list`: proves the local MCP server schema exposes tools.
- Active model tool surface: proves the current Codex session can actually call those tools.
...
This records a redacted issue and writes `development/tool-exposure-diagnostic-report.md` for later MCP development.
...
```text
development/spawn-agent-diagnostic-report.md
```

The script can also write main-agent-authored Markdown from stdin:
...
9. If observable MCP tools are absent from the active model tool surface, run `diagnose_tool_exposure.js`, report the diagnosis, and stop unless the human explicitly accepts unobservable fallback.
